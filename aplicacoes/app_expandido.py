...
# -*- coding: utf-8 -*-
"""
Aplicação Web CARF ML Predictor - Versão Expandida
Inclui probabilidade de provimento e tipo de votação
"""
...
st.set_page_config(
    page_title="CARF ML Predictor Expandido",
    page_icon="⚖️",
    layout="wide"
)
...
def load_models_and_preprocessors():
    ...
    try:
        # Carregar modelos
        model_provimento = joblib.load('modelo_carf_provimento.pkl')
        model_votacao = joblib.load('modelo_carf_votacao.pkl')
        
        # Carregar pré-processadores
        with open('preprocessors_expandido.pkl', 'rb') as f:
...
def prever_probabilidades_expandido(texto_ementa, tributo, turma, model_provimento, model_votacao, preprocessors):
...
def main():
    ...
    # Título principal
    st.title("⚖️ CARF ML Predictor Expandido")
    ...
    if model_provimento is None or model_votacao is None or preprocessors is None:
        ...
        st.info("Execute primeiro: `cd notebooks && python train_model_expandido.py`")
        return
    ...
    **Modelo de Votação:**
    ...
    Ambos utilizam Random Forest + TF-IDF
    """)
    ...
    if st.button("🔮 Prever Probabilidades", type="primary"):
        if texto_ementa.strip():
            with st.spinner("Processando predição..."):
                prob_provimento, prob_votacao, erro = prever_probabilidades_expandido(
                    texto_ementa, 
                    tributo_selecionado, 
                    turma_selecionada,
                    model_provimento,
            ...
            if erro:
                ...
                with col2:
                    ...
                    # Métricas de votação
                    col_vot1, col_vot2, col_vot3 = st.columns(3)
